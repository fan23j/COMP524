...
(define (parse-invocation-tail)
  ;; invocationTail ::= DEFINE NAME expr CPAREN | exprList CPAREN
  (if (check 'DEFINE)
      (list 'invocationTail
            (consume 'DEFINE)
            (consume 'NAME)
...
(define (parse-invocation)
  ;; invocation ::= OPAREN invocationTail
  (list 'invocation
        (consume 'OPAREN)
...
(define (parse2 code)
  (parameterize ([tokens (lex2 code)])
    (parse-program)))

(module+ test ;; smaller integration tests
  (check-equal? (parse2 "7")
                '(program
                  (exprList
                   (expr
                    (atom
  ...
  ;; definition
  (check-equal? (parse2 "(define foo 7)")
                '(program
                  (exprList
                   (expr
                    (invocation
                     ...
                     (invocationTail
                      (DEFINE #f)
                      (NAME foo)
                      (expr (atom (number (INT 7))))
                      (CPAREN #f))))
                   (optExprList))))

  ;; invocation
  (check-equal? (parse2 "(list)")
                '(program
                  (exprList
                   (expr
                    (invocation
